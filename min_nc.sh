#!/bin/bash
# min_nc.sh version 1.01

  #********************************************************************#
  #                                                                    #
  # Program to compute minimum for each monthly file.nc.               #
  ...
  for ((year=syear; year <= eyear; year++))  # year counter
   do
     ...
     do
       ...
       file_Out=ww3.${yyyymm}_min_${var1}.nc
       ...
       then
         ...
         ncwa -t 4 -y min -v ${var1} -a time ${fileIn} ${file_Out}
